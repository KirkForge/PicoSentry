...
1. **Plan**: write `workplan.md` (scratch) — files to touch, root cause (not symptom), gate to run. Substantial named work gets a WO file in `docs/workorders/` (`WO<major>.<minor>.<patch>-NNN-slug.md`, headers: Series/Status/Owner/Scope/Gate/Objective). Next free series: **WO4.0.0**.
