...
**Status:** CLOSED-FOLDED (2026-08-18) — remainder (HMAC-with-primary rotation announcements, check_hostname ceiling, JSONL compaction) moved to **WO5.0.0-030**; the gossip-401 auth seam was separately fixed as WO5.0.0-004 (2026-08-18). Landed & shipped in v2.1.2: digest-only gossip snapshots (no secret material; legacy raw snapshots still mergeable for rolling upgrades), slow-cadence OFFLINE-peer re-probe + heal via merge, scheduled retire_stale_tokens (PICODOME_CLUSTER_TOKEN_GRACE_SECONDS), scheduled retention run_cleanup, JSONL runtime max_jobs cap, SQLite queries inside the lock, Redis zset prune. Evidence: tests/sandbox/test_cluster_trust_healing.py, updated test_cluster.py / test_cluster_3node.py
