"""Shared audit primitives for the Pico Security Series.

Provides the shared base types for audit logging across all PicoSeries
codebases. Each codebase extends these with its own sink implementations
and domain-specific event types.
...
- HashChainedMixin: hash-chained audit log integrity (from PicoDome pattern)
...
logger = logging.getLogger("pico_core.audit")
...
@dataclass
class AuditEvent:  # rationale: structured audit event schema shared across all PicoSeries codebases
...
class AuditSinkBase(ABC):  # rationale: abstract sink interface for audit logging, extensible per codebase
...
class HashChainedMixin:  # rationale: tamper-evident audit log via hash chaining, extracted from PicoDome pattern
