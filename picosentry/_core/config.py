"""Shared configuration primitives for the Pico Security Series.
...
All PicoSeries codebases implement from_env() and assert_secure()
independently. This module provides the shared core so each codebase
can register its own checks while using the same gate mechanism.
"""
...
logger = logging.getLogger("pico_core.config")


# ── from_env helper ──────────────────────────────────────────────────────


def from_env(key: str, default: str | None = None, required: bool = False) -> str | None:  # rationale: shared env var reader, used by all 4 codebase configs
...
def assert_secure(  # rationale: startup security gate, standardized exit code 7 across all codebases
