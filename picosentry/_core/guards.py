...
Shared across PicoDome and PicoSentry. The core thesis: same inputs + same
policy = same output, every time.
...
class DeterministicGuard:  # rationale: runtime determinism enforcement, shared between PicoDome and PicoSentry
...
def deterministic_hash(data: dict[str, Any], exclude_fields: tuple[str, ...] = ("run_id", "timestamp", "duration_ms", "scan_id")) -> str:  # rationale: SHA-256 of deterministic fields only, excludes timing for reproducibility
...
def verify_determinism(hash_a: str, hash_b: str) -> tuple[bool, str, str]:  # rationale: CI gate — compare two results for identical output
