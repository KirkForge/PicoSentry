...
COMPONENT_STATUS: tuple[ComponentStatus, ...] = (
    ComponentStatus(
        ...
        status="Stable",
        notes="Core scanner; 7 ecosystems; deterministic, offline; 50 rules, 6495 fixtures",
    ),
    ComponentStatus(
    ...
    ComponentStatus(
        ...
        status="Stable",
        notes="6495 fixtures (5558 pos / 930 neg), 50 rules, 94.44% prec, 68.89% recall — see docs/model-card.md",
    ),
    ComponentStatus(
        name="Docker image",
        status="Stable",
        notes="`kirkforge/picodome:v2.1.1` on Docker Hub; multi-arch (linux/amd64 + linux/arm64); non-root user",
    ),
    ComponentStatus(
        name="PyPI package",
        status="Stable",
        notes="`pip install picosentry` — v2.1.1 published",
    ),
)
