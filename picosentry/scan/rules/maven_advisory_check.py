"""
L2-MAVEN-ADV-001: Maven artifact advisory database vulnerability detection.

Checks Java/Gradle dependencies against a local OSV-format advisory database.
Flags artifacts with known CVEs or security advisories.

Pure function: (target_path, corpus_dir) -> List[Finding]

Follows the same pattern as Go/npm/Cargo advisory check but for the Maven ecosystem.
Maven artifacts use the "Maven" ecosystem in OSV format.
"""
...
logger = logging.getLogger("picosentry.maven_advisory_check")

__all__ = ["detect_maven_advisory_vulnerabilities"]
...
def _get_maven_packages(target: Path) -> list[tuple[str, str, str, Path]]:
    """Collect all (name, version, label, source_path) tuples for a Maven project."""
    ...
    if pom_data:
        for dep in pom_data.get("dependencies", []):
            group_id, artifact_id, version = dep[0], dep[1], dep[2]
            pkg_key = f"{group_id}:{artifact_id}"
            if pkg_key and (pkg_key, version) not in seen:
                seen.add((pkg_key, version))
                packages.append((pkg_key, version, f"{artifact_id}@{version}", target / "pom.xml"))

    gradle_data = parse_gradle_build(target)
    if gradle_data:
        for dep in gradle_data.get("dependencies", []):
            group, artifact, version = dep[0], dep[1], dep[2]
            pkg_key = f"{group}:{artifact}"
            if pkg_key and version and (pkg_key, version) not in seen:
                seen.add((pkg_key, version))
                packages.append((pkg_key, version, f"{artifact}@{version}", target / "build.gradle"))

    return packages


def detect_maven_advisory_vulnerabilities(
    target: Path, corpus_dir: Path, advisory_db_path: str | None = None
) -> list[Finding]:
    """
    Detect Maven artifacts with known security advisories.
    ...
    if db is None:
        logger.debug("No advisory DB loaded — skipping Maven advisory check")
        return findings

    packages = _get_maven_packages(target)

    for pkg_key, pkg_version, pkg_label, source_path in packages:
        advisories = db.check(pkg_key, pkg_version)
        if not advisories:
            continue
        ...
        for adv in advisories:
            ...
            findings.append(
                Finding(
                    ...
                    remediation=f"Vulnerability in {pkg_key}@{pkg_version}.{fixed_hint} See {adv.references[0] if adv.references else 'advisory database'} for details.",
