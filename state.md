...
- **P4 #10 status refresh in Gap Audit.** Updated the broad-exception audit
  ...
  service). Reduced remaining site count to ~153.
- **state.md cleanup.** Reconciled the current session header, "Still open"
  ...
  agree that security-relevant exception narrowing is closed, ~153 broad
...
- **P4 #10 broad exception audit — SECURITY-RELEVANT SLICES CLOSED.** All
  ...
  loading paths, sandbox health/readiness probes, and baseline hardening
  audit logging have been narrowed to specific exception types with
  regression tests.
  **Remaining:** ~153 broad `except Exception` sites across the codebase are
...
2. **Continue opportunistic P4 #10 narrowing** on `no-ci/*` branches for the
   remaining ~153 broad `except Exception` sites, prioritizing request-boundary,
...
middleware/server/rate_limit/DB manager, serve routers, backup service,
serve log/alert services, serve execution/observability, plugin manager
loading paths, sandbox health/readiness, and baseline hardening audit
logging slices all narrowed to specific exception types + logged. Only
intentional boundaries remain (`plugin_worker.py` RPC loop,
`database/pools.py` deliberate rollback+re-raise). **Live count:** ~153
