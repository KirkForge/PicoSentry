"""WO5.0.0-031 gate: serve multi-worker readiness.

Two layers, honestly labeled:

1. In-process tests exercise the cross-worker PROTOCOLS directly against the
   shared sqlite DB: two JobScheduler instances racing for one lease
   (exactly-one-fire + takeover), two RateLimitMiddleware instances sharing
   persisted counters, outbox publish→poll fanout onto a foreign bus, and
   WS slow-consumer isolation.
2. ONE end-to-end test boots TWO real uvicorn server processes against one
   shared sqlite DB — the actual production multi-worker topology (separate
   processes, zero shared memory) — and proves through real HTTP/WS:
   boot-time add_job convergence (simultaneous boot crashed on base with
   UNIQUE constraint failure), exactly one scheduler leader across the two
   processes, event fanout from worker A's API to worker B's WS + event
   history, and rate limits counted across workers.

   What this file does NOT prove: cron double-fire over real minute
   boundaries (the in-process lease tests cover the mechanism without
   wall-clock waits) and metrics aggregation (DECISION: per-worker
   /metrics, aggregation belongs to the scraper — see
   picosentry/serve/services/metrics.py docstring).
"""

from __future__ import annotations

import asyncio
import httpx
import itertools
import json
import os
import socket
import sqlite3
import subprocess
import sys
import time
from datetime import datetime, timedelta
from pathlib import Path

import pytest

from picosentry.serve.middleware.rate_limit import RateLimitMiddleware
from picosentry.serve.services.event_bus import EventBus, OutboxPoller
from picosentry.serve.services.scheduler import JobScheduler, _utcnow
from picosentry.serve.services.websocket_manager import ConnectionManager


def _wait_until(predicate, timeout: float, interval: float = 0.05) -> bool:
    """Poll a predicate with small injected waits; no unbounded sleeps."""
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        if predicate():
            return True
        time.sleep(interval)
    return predicate()


# ---------------------------------------------------------------------------
# (b) scheduler leader lease — in-process, two instances, one shared DB
# ---------------------------------------------------------------------------


class _LeaseClock:
    """Controllable replacement for scheduler._utcnow (no wall sleeps)."""

    def __init__(self) -> None:
        self.now = _utcnow()

    def __call__(self) -> datetime:
        return self.now

    def advance(self, seconds: float) -> None:
        self.now += timedelta(seconds=seconds)


def _fresh_scheduler(lease_ttl: int = 15, tick_sleep: float = 0.05) -> JobScheduler:
    sched = JobScheduler()
    sched.lease_ttl = lease_ttl
    sched._tick_sleep = tick_sleep
    return sched


def test_lease_held_exclusively_and_taken_over_on_expiry(monkeypatch) -> None:
    clock = _LeaseClock()
    monkeypatch.setattr("picosentry.serve.services.scheduler._utcnow", clock)
    a = _fresh_scheduler(lease_ttl=10)
    b = _fresh_scheduler(lease_ttl=10)

    assert a._try_acquire_lease() is True  # seed row is long expired
    assert b._try_acquire_lease() is False  # a holds it
    assert a._try_acquire_lease() is True  # holder renews unconditionally

    clock.advance(11)  # lease expired: a stopped heartbeating (crashed)
    assert b._try_acquire_lease() is True  # standby takes over
    assert a._try_acquire_lease() is False  # the old holder is demoted

    b._release_lease()
    assert a._try_acquire_lease() is True  # released lease is acquirable again


def test_two_schedulers_fire_job_exactly_once_and_take_over(monkeypatch, tmp_path) -> None:
    """The cross-worker single-fire gate: two scheduler instances (=
    two workers) share one DB; only the lease holder's queue entries fire,
    and a takeover after lease expiry keeps firing exactly-once."""
    a = _fresh_scheduler(lease_ttl=2, tick_sleep=0.05)
    b = _fresh_scheduler(lease_ttl=2, tick_sleep=0.05)

    fires: list[str] = []
    for sched in (a, b):
        counter = itertools.count()

        def fake_next_run(cron: str, _c=counter) -> datetime:
            # Calls 0-2: add_job validation, start()'s initial schedule and
            # the leadership-gain reschedule (which cancel-and-replaces the
            # initial entry — recomputing next_run from cron, identical in
            # production). Those arm the near fire under test. Later calls
            # arm an hour out so each leadership epoch fires at most once.
            return datetime.now() + (timedelta(milliseconds=100) if next(_c) < 3 else timedelta(hours=1))

        sched._get_next_run = fake_next_run  # type: ignore[method-assign]
        sched._execute_job = lambda job_id, _s=sched: fires.append(_s.worker_id)  # type: ignore[method-assign]

    a.add_job(name=f"lease_probe_{int(time.time() * 1000)}", cron="* * * * *", command="cleanup", params={})
    a.start()
    b.start()
    try:
        assert _wait_until(lambda: len(fires) >= 1, timeout=3), f"no fire recorded: {fires}"
        time.sleep(0.3)  # let the standby's tick land (would double-fire on base)
        assert len(fires) == 1, f"double fire across workers: {fires}"
        assert (a.is_leader() + b.is_leader()) == 1, "exactly one leader"

        # Takeover: the leader stops (releases its lease); the standby
        # acquires, drops its stale overdue entry (the transition
        # reschedule), arms fresh and fires the job exactly once itself.
        if a.is_leader():
            a.stop()
            leader = b
        else:
            b.stop()
            leader = a
        assert _wait_until(leader.is_leader, timeout=5), "standby never took over the lease"
        assert _wait_until(lambda: len(fires) >= 2, timeout=5), f"takeover did not fire: {fires}"
        time.sleep(0.3)
        assert len(fires) == 2, f"takeover double-fired: {fires}"
        assert fires[1] == leader.worker_id
    finally:
        a.stop()
        b.stop()


def test_add_job_atomic_under_concurrent_boot(tmp_path) -> None:
    """Simultaneous lifespan add_job() from two workers must converge, not
    crash the loser on the name UNIQUE constraint (verified on base)."""
    a = _fresh_scheduler()
    b = _fresh_scheduler()
    name = f"boot_race_{int(time.time() * 1000)}"

    job_a = a.add_job(name=name, cron="* * * * *", command="cleanup", params={})
    job_b = b.add_job(name=name, cron="* * * * *", command="cleanup", params={})
    assert job_a == job_b  # both converged on the same row


# ---------------------------------------------------------------------------
# (a-lite) outbox fanout onto a foreign worker's bus — in-process
# ---------------------------------------------------------------------------


def test_outbox_fanout_to_foreign_bus() -> None:
    bus_a = EventBus()
    bus_a.outbox_enabled = True
    bus_b = EventBus()
    seen: list = []
    bus_b.subscribe("multi.test", seen.append)

    poller = OutboxPoller(bus_b, interval=0.05, retention_seconds=3600)
    poller.start()
    try:
        event = bus_a.publish("multi.test", {"v": 1}, source="test")
        assert _wait_until(lambda: len(seen) == 1, timeout=3), f"foreign dispatch missing: {seen}"
        assert seen[0].id == event.id
        assert seen[0].payload == {"v": 1}
        assert any(e.id == event.id for e in bus_b.get_history("multi.test", limit=50))

        # Own-worker events must not loop back: bus_b's own publish is
        # dispatched synchronously and skipped by its poller.
        own = bus_b.publish("multi.test", {"v": 2}, source="b")
        time.sleep(0.3)  # several poll intervals
        assert len([e for e in seen if e.id == own.id]) == 1
    finally:
        poller.stop()


# ---------------------------------------------------------------------------
# (c-lite) rate limit counted across two middleware instances — in-process
# ---------------------------------------------------------------------------


def test_rate_limit_counters_shared_across_two_instances() -> None:
    m1 = RateLimitMiddleware(app=None, max_requests_per_ip=3, window=60, persist=True, sync_interval=0.1)
    m2 = RateLimitMiddleware(app=None, max_requests_per_ip=3, window=60, persist=True, sync_interval=0.1)
    key = f"mw-{time.time()}-{os.getpid()}"

    now = time.time()
    for i in range(3):
        limited, _ = m1._record_and_check("ip", key, 3, now + i * 0.001, m1.ip_requests)
        assert not limited
    limited, _ = m1._record_and_check("ip", key, 3, now + 0.01, m1.ip_requests)
    assert limited, "worker 1 must be limited after its own 3 requests"

    # Beyond the sync window, worker 1's next request flushes its counts to
    # the shared table and worker 2's next request pulls them in: the same
    # client is limited on the other worker too. (Flushing is lazy — it
    # rides on the next request, which is the production behavior.)
    time.sleep(0.25)
    m1._record_and_check("ip", key, 3, now + 0.3, m1.ip_requests)
    limited, _ = m2._record_and_check("ip", key, 3, now + 0.31, m2.ip_requests)
    assert limited, "worker 2 did not see worker 1's counted requests"


# ---------------------------------------------------------------------------
# (d) WS slow consumer never blocks a fast consumer — in-process
# ---------------------------------------------------------------------------


class _SlowSocket:
    def __init__(self, delay: float) -> None:
        self.delay = delay
        self.sent: list[str] = []
        self.busy = False

    async def accept(self) -> None:
        pass

    async def send_text(self, message: str) -> None:
        self.busy = True
        await asyncio.sleep(self.delay)
        self.sent.append(message)
        self.busy = False


@pytest.mark.asyncio
async def test_slow_ws_consumer_does_not_block_fast_consumer() -> None:
    manager = ConnectionManager()
    manager.BROADCAST_DRAIN_TIMEOUT = 0.1
    fast, slow = _SlowSocket(delay=0.0), _SlowSocket(delay=0.4)
    await manager.connect(fast, channels=["*"], org_id=None)
    await manager.connect(slow, channels=["*"], org_id=None)

    started = time.monotonic()
    await manager.broadcast("multi.ws", {"n": 1})
    elapsed = time.monotonic() - started

    # The fast client's delivery must not wait for the slow one's 0.4s send
    # (broadcast itself gives up waiting at the 0.1s drain cap).
    assert elapsed < 0.35, f"broadcast serialized behind the slow consumer ({elapsed:.2f}s)"
    assert len(fast.sent) == 1, "fast consumer was not served first"

    await asyncio.sleep(0.45)  # slow send completes on its own schedule
    assert len(slow.sent) == 1


# ---------------------------------------------------------------------------
# end-to-end: two REAL uvicorn workers on one shared sqlite DB
# ---------------------------------------------------------------------------

_SUBPROC_TIMEOUT = 90.0


def _free_port() -> int:
    with socket.socket() as s:
        s.bind(("127.0.0.1", 0))
        return s.getsockname()[1]


class _Workers:
    def __init__(self, tmp_path: Path) -> None:
        self.db_path = tmp_path / "shared.db"
        self.ports = [_free_port(), _free_port()]
        self.procs: list[subprocess.Popen] = []
        self.logs: list[Path] = []

    def env(self) -> dict[str, str]:
        env = dict(os.environ)
        env.update(
            {
                "PICOSHOGUN_DATABASE_PATH": str(self.db_path),
                "PICOSHOGUN_DATABASE_JOURNAL_MODE": "WAL",
                "PICOSHOGUN_EVENT_OUTBOX": "true",
                "PICOSHOGUN_EVENT_OUTBOX_POLL": "0.1",
                "PICOSHOGUN_API_WORKERS": "2",
                "PICOSHOGUN_RATE_LIMIT_SYNC_SECONDS": "0.3",
                "PICOSHOGUN_SCHEDULER_LEASE_TTL": "5",
                "PICOSHOGUN_LOG_STRUCTURED": "false",
                "PICOSHOGUN_BACKUP_DIR": "",
            }
        )
        return env

    def start(self) -> None:
        for i, port in enumerate(self.ports):
            log = self.db_path.parent / f"worker-{port}.log"
            self.logs.append(log)
            self.procs.append(
                subprocess.Popen(
                    [
                        sys.executable,
                        "-m",
                        "uvicorn",
                        "picosentry.serve.api.server:app",
                        "--host",
                        "127.0.0.1",
                        "--port",
                        str(port),
                        "--log-level",
                        "warning",
                    ],
                    stdout=open(log, "w"),
                    stderr=subprocess.STDOUT,
                    env=self.env(),
                    cwd=str(Path(__file__).resolve().parents[2]),
                )
            )
        deadline = time.monotonic() + _SUBPROC_TIMEOUT
        for port in self.ports:
            ok = False
            while time.monotonic() < deadline:
                if self.procs and any(p.poll() is not None for p in self.procs):
                    pytest.fail(f"a worker died at boot; log:\n{self.dump()}")
                try:
                    response = httpx.get(f"http://127.0.0.1:{port}/health/live", timeout=1.0)
                    ok = response.status_code == 200
                except httpx.HTTPError:
                    ok = False
                if ok:
                    break
                time.sleep(0.3)
            assert ok, f"worker on {port} never became healthy; log:\n{self.dump()}"

    def dump(self) -> str:
        parts = []
        for log in self.logs:
            if log.exists():
                parts.append(f"--- {log.name} (tail) ---\n" + "\n".join(log.read_text().splitlines()[-40:]))
        return "\n".join(parts)

    def stop(self) -> None:
        for proc in self.procs:
            proc.terminate()
        for proc in self.procs:
            try:
                proc.wait(timeout=10)
            except subprocess.TimeoutExpired:
                proc.kill()


@pytest.fixture(scope="module")
def workers(tmp_path_factory) -> _Workers:
    w = _Workers(tmp_path_factory.mktemp("multiworker"))
    w.start()
    yield w
    w.stop()


def _seed_admin(workers: _Workers) -> dict:
    """Register a viewer via worker A, promote to admin directly in the
    shared DB, then log in (register endpoint only creates viewers)."""
    base_a = f"http://127.0.0.1:{workers.ports[0]}"
    username = f"mw_admin_{int(time.time() * 1000)}"
    password = "MultiWorker123!"
    with httpx.Client(base_url=base_a, timeout=10) as c:
        r = c.post("/auth/register", json={"username": username, "password": password})
        assert r.status_code == 201, r.text

    conn = sqlite3.connect(str(workers.db_path), timeout=30)
    try:
        conn.execute("UPDATE users SET role = 'admin' WHERE username = ?", (username,))
        conn.commit()
    finally:
        conn.close()

    with httpx.Client(base_url=base_a, timeout=10) as c:
        r = c.post("/auth/login", json={"username": username, "password": password})
        assert r.status_code == 200, r.text
        token = r.json()["access_token"]
        headers = {"Authorization": f"Bearer {token}"}
        r = c.post("/orgs", json={"name": "MultiWorker Org", "slug": f"mw-{int(time.time() * 1000)}"}, headers=headers)
        assert r.status_code == 201, r.text
    return {"token": token, "headers": headers}


def test_two_real_workers_fanout_leader_and_rate_limits(workers: _Workers) -> None:
    import websockets

    base = [f"http://127.0.0.1:{p}" for p in workers.ports]
    admin = _seed_admin(workers)
    headers = admin["headers"]

    # --- exactly one scheduler leader across the two processes -----------
    with httpx.Client(base_url=base[0], timeout=10) as c:
        r = c.post(
            "/scheduler/jobs",
            json={"name": f"leader_probe_{int(time.time() * 1000)}", "cron": "* * * * *", "command": "cleanup"},
            headers=headers,
        )
        assert r.status_code == 201, r.text
        r = c.get("/scheduler/jobs", headers=headers)
        jobs_a = r.json()["jobs"]
    with httpx.Client(base_url=base[1], timeout=10) as c:
        r = c.get("/scheduler/jobs", headers=headers)
        jobs_b = r.json()["jobs"]
    leaders = {job["leader"] for job in jobs_a + jobs_b if job["name"].startswith("leader_probe")}
    # Same job listed by both workers; the leader flag is PER WORKER.
    leader_flags = [job["leader"] for job in jobs_a if job["name"].startswith("leader_probe")]
    leader_flags_b = [job["leader"] for job in jobs_b if job["name"].startswith("leader_probe")]
    assert leader_flags and leader_flags_b, "probe job missing from a worker's status"
    assert sum(bool(f) for f in leader_flags + leader_flags_b) == 1, (
        f"expected exactly one leader across both workers, got A={leader_flags} B={leader_flags_b}"
    )

    # --- (a) event published via worker A reaches worker B's WS ----------
    async def _fanout() -> str | None:
        uri = f"ws://127.0.0.1:{workers.ports[1]}/ws"
        async with websockets.connect(uri, open_timeout=10) as ws:
            await ws.send(json.dumps({"action": "auth", "token": admin["token"]}))
            ack = json.loads(await asyncio.wait_for(ws.recv(), timeout=5))
            assert ack["type"] == "auth", ack
            await ws.send(json.dumps({"action": "subscribe", "channels": ["*"]}))
            sub = json.loads(await asyncio.wait_for(ws.recv(), timeout=5))
            assert sub["type"] == "subscribed", sub

            async with httpx.AsyncClient(base_url=base[0], timeout=20) as c:
                trig = asyncio.create_task(
                    c.post(
                        "/projects/picosentry/run", json={"project_id": "picosentry", "timeout": 10}, headers=headers
                    )
                )
                deadline = time.monotonic() + 15
                while time.monotonic() < deadline:
                    try:
                        msg = json.loads(await asyncio.wait_for(ws.recv(), timeout=1))
                    except (asyncio.TimeoutError, TimeoutError):
                        continue
                    if msg.get("type", "").startswith("project.run"):
                        await trig
                        return msg["payload"]["run_id"]
                await trig
                return None

    run_id_on_b = asyncio.run(_fanout())
    workers_dump = workers.dump()
    assert run_id_on_b is not None, f"worker B's WS never saw worker A's run event; logs:\n{workers_dump}"

    # --- dashboard parity: both workers' history contain the event -------
    run_events = []
    for base_url in base:
        with httpx.Client(base_url=base_url, timeout=10) as c:
            r = c.get("/events/history?event_type=project.run.started&limit=50", headers=headers)
            assert r.status_code == 200, r.text
            run_events.append([e["payload"].get("run_id") for e in r.json()])
    assert run_id_on_b in run_events[0], f"worker A history missing its own run: {run_events}"
    assert run_id_on_b in run_events[1], f"worker B history missing worker A's run (fanout gap): {run_events}"

    # --- (c) rate limit counted across workers ---------------------------
    # /alerts is NOT a DDoS-shield high-risk path, so any 429 here comes
    # from the (shared) RateLimitMiddleware, isolating the cross-worker
    # counting proof from the per-process shield.
    with httpx.Client(base_url=base[0], timeout=10) as c:
        last = None
        for _ in range(105):
            r = c.get("/alerts", headers=headers)
            last = r.status_code
            if last == 429:
                break
        assert last == 429, "worker A never hit the per-IP limit (unexpected)"
    time.sleep(1.0)  # > sync window: worker B pulls A's timestamps
    with httpx.Client(base_url=base[1], timeout=10) as c:
        r = c.get("/alerts", headers=headers)
    assert r.status_code == 429, (
        f"worker B did not share worker A's rate-limit count (got {r.status_code}); logs:\n{workers_dump}"
    )
